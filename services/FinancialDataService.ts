...
export class FinancialDataService {
  ...
  /**
   * Fetch payments for a billing month for bank reconciliation export.
   * Returns rows with Date, Amount, Reference, Student, Parent, Category.
   */
  static async getPaymentsForBankReconciliation(
    orgId: string,
    monthIso: string,
  ): Promise<
    Array<{
  ...
  > {
    const supabase = assertSupabase();
    const month = this.monthStartIsoFromValue(monthIso, { recoverUtcMonthBoundary: true }) || monthIso;
    ...
    for (const p of payments) {
      const accountingMonth = this.resolvePaymentAccountingMonth(p);
      if (accountingMonth !== month) continue;
      ...
      const categoryCode = inferFeeCategoryCode(
        p?.category_code || metadata?.category_code || metadata?.fee_category || p?.description || 'tuition',
      );
      const categoryLabel = this.CATEGORY_LABELS[categoryCode] || categoryCode;
      ...
      const ref = this.normalizeReference(
      ...
      rows.push({
        ...
        student,
        parent: '', // Parent name not in payments join; could extend later
