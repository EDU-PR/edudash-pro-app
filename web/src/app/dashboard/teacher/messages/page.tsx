...
const TeacherThreadItem = ({ thread, isActive, onSelect, currentUserId }: ThreadItemProps) => {
  ...
  const getMessageStatus = () => {
    ...
    if (isRead) return { ticks: '✓✓', color: '#34d399' }; // Green double tick for read
...
function TeacherMessagesPage() {
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          // Non-critical; sender-side ticks will still update once recipient marks delivered later.
        }
      }
  ...
  const fetchThreads = useCallback(async () => {
    ...
    try {
      ...
      const threadsWithDetails = teacherThreads.map((thread: any) => {
      ...
      const response = await fetch('/api/ai-proxy', {
        ...
        body: JSON.stringify({
          ...
          payload: {
            prompt: content.trim(),
            context:
              'You are Dash, an AI teaching assistant for teachers. Keep replies practical and concise. When helpful, offer templates (lesson plan, parent message) and ask one short clarifying question.',
            conversationHistory: dashAIMessages.slice(-10).map((m) => ({
              role: m.sender_id === DASH_AI_USER_ID ? 'assistant' : 'user',
              content: m.content,
            })),
          },
          metadata: { role: 'teacher', source: 'teacher_messages_dash_ai' },
        }),
      });
