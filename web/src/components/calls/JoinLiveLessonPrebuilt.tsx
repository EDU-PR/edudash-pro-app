...
import { Video, ChevronRight, Loader2, Radio, Bell, AlertCircle, RefreshCw, LogIn } from 'lucide-react';
...
interface JoinLiveLessonPrebuiltProps {
  studentId?: string;
  classId?: string;
  preschoolId: string;
}

function JoinLiveLessonPrebuiltInner({ classId, preschoolId }: JoinLiveLessonPrebuiltProps) {
  ...
  // Fetch live lessons function
  const fetchLiveLessons = async (isInitial: boolean = false) => {
    if (isInitial) setLoading(true);

    console.log('[JoinLiveLessonPrebuilt] Fetching live lessons for preschool:', preschoolId, 'class:', classId);

    const now = new Date().toISOString();

    // First, clean up any expired calls
    try {
      await supabase
        .from('video_calls')
      ...
      console.warn('[JoinLiveLessonPrebuilt] Cleanup error:', e);
    }
    ...
    console.log('[JoinLiveLessonPrebuilt] Fetched lessons:', data?.length || 0, error ? `Error: ${error.message}` : '');
  ...
  useEffect(() => {
    ...
    console.log('[JoinLiveLessonPrebuilt] Setting up realtime subscription for preschool:', preschoolId);

    const channel = supabase
      .channel(`live-lessons-prebuilt-${preschoolId}-${Date.now()}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload: { new: Record<string, unknown> }) => {
          console.log('[JoinLiveLessonPrebuilt] Realtime INSERT:', payload.new);
      ...
      .on(
        ...
        (payload: { new: Record<string, unknown> }) => {
          console.log('[JoinLiveLessonPrebuilt] Realtime UPDATE:', payload.new);
          fetchLiveLessons(false);
        }
      )
      .on(
        ...
        (payload: { old: Record<string, unknown> }) => {
          console.log('[JoinLiveLessonPrebuilt] Realtime DELETE:', payload.old);
          fetchLiveLessons(false);
        }
      )
  ...
  if (showError) {
    const isAuthError =
      showError.toLowerCase().includes('sign in') ||
      ...
      >
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: 'white' }}>Unable to Join</h3>
              <p style={{ margin: '4px 0 0', fontSize: 14, color: 'rgba(255, 255, 255, 0.8)' }}>{showError}</p>
            </div>
          </div>
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: 'clamp(16px, 4vw, 20px)', fontWeight: 700, color: 'white' }}>Live Lessons</h3>
...
export function JoinLiveLessonPrebuilt(props: JoinLiveLessonPrebuiltProps) {
  return (
    <DailyPrebuiltProvider>
      <JoinLiveLessonPrebuiltInner {...props} />
    </DailyPrebuiltProvider>
  );
}
